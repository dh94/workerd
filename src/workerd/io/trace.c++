...
tracing::Attribute::Attribute(kj::ConstString name, kj::OneOf<Value, kj::Array<Value>>&& value)
    : name(kj::mv(name)),
      value(kj::mv(value)) {}
  ...
  static auto readValue =
    ...
    switch (inner.which()) {
      case rpc::Trace::Attribute::Value::Inner::TEXT: {
        return kj::ConstString(kj::str(inner.getText()));
      }
...
tracing::Attribute::Attribute(rpc::Trace::Attribute::Reader reader)
    : name(kj::ConstString(kj::str(reader.getName()))),
...
tracing::Attribute tracing::Attribute::clone() {
  static constexpr auto cloneValue = [](const Value& value) -> Value {
    KJ_SWITCH_ONEOF(value) {
      KJ_CASE_ONEOF(str, kj::ConstString) {
  ...
  KJ_SWITCH_ONEOF(value) {
    KJ_CASE_ONEOF(value, Value) {
      return Attribute(kj::ConstString(kj::str(name)), cloneValue(value));
    }
    KJ_CASE_ONEOF(values, kj::Array<Value>) {
      return Attribute(
          kj::ConstString(kj::str(name)), KJ_MAP(value, values) { return cloneValue(value); });
    }
  }
  KJ_UNREACHABLE;
}
...
tracing::SpanOpen::SpanOpen(kj::Maybe<kj::ConstString> operationName, kj::Maybe<Info> info)
    : operationName(kj::mv(operationName)),
      info(kj::mv(info)) {}

namespace {
kj::Maybe<kj::ConstString> readSpanOpenOperationName(rpc::Trace::SpanOpen::Reader& reader) {
  if (!reader.hasOperationName()) return kj::none;
  return kj::ConstString(kj::str(reader.getOperationName()));
}
...
tracing::SpanOpen tracing::SpanOpen::clone() {
  static constexpr auto cloneInfo =
      [](kj::Maybe<Info>& info) -> kj::Maybe<tracing::SpanOpen::Info> {
  ...
  return SpanOpen(
      operationName.map([](auto& str) { return kj::ConstString(kj::str(str)); }), cloneInfo(info));
}
...
tracing::Onset::Onset(tracing::Onset::Info&& info, ExecutionModel executionModel)
    : info(kj::mv(info)),
      executionModel(executionModel) {}
...
tracing::Onset tracing::Onset::clone() {
  static constexpr auto cloneInfo = [](Info& info) -> tracing::Onset::Info {
    KJ_SWITCH_ONEOF(info) {
      KJ_CASE_ONEOF(fetch, FetchEventInfo) {
        return fetch.clone();
      }
...
tracing::TailEvent tracing::TailEvent::clone() {
  static constexpr auto cloneEvent = [](Event& event) -> Event {
    KJ_SWITCH_ONEOF(event) {
      KJ_CASE_ONEOF(onset, Onset) {
        return onset.clone();
      }
